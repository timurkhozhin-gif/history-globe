...
### Как обновить сайт (текущий способ — WinSCP)

1. Открыть **WinSCP**
2. Подключиться: хост `108.165.174.59`, пользователь `root`, порт `22`
3. Перейти в правой панели в `/var/www/globe/`
4. Выделить все новые файлы слева и перетащить вправо, согласившись на замену

### Как обновить сайт (быстрый способ — Git, TODO)

Когда проект будет на GitHub, обновление одной командой по SSH:
```bash
cd /var/www/globe && git pull
```
